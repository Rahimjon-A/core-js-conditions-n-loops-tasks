...
function canQueenCaptureKing(queen, king) {
  ...
  const korX = (queen.x - king.x) ** 2;
  const korY = (queen.y - king.y) ** 2;
...
function isContainNumber(num, digit) {
  ...
  for (let i = 0; i < digitCount; i += 1) {
    const qoldiq = myDigit % 10;

    if (qoldiq === digit) {
      return true;
    }
    myDigit = (myDigit - qoldiq) / 10;
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    let front = '';
    let end = '';
    for (let j = 0; j < myStr.length; j += 1) {
      if (j % 2 === 0) {
        front += myStr[j];
      } else {
        end += myStr[j];
...
function getNearestBigger(number) {
  ...
  let result = 0;
  for (let q = 0; q < digits.length; q += 1) {
